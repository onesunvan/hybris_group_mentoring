    ...
    <div class="nav_column applied-filters"> <!-- changed for telco -->
        ...
        <div class="item">
            <ul class="facet_block">
                <li>
                    <c:url value="/search?text=${ycommerce:encodeUrl(pageData.freeTextSearch)}"
                           var="freeTextSearchUrl"/>
                    <a href="${freeTextSearchUrl}">${pageData.freeTextSearch}</a>
                </li>
                <c:forEach items="${pageData.breadcrumbs}" var="breadcrumb">
                    <li>
                        <c:url value="${breadcrumb.removeQuery.url}" var="removeQueryUrl"/>
                        <a href="${removeQueryUrl}"><spring:theme code="search.nav.appliedFacet"
                                                                  arguments="${breadcrumb.facetName}^${breadcrumb.facetValueName}"
                                                                  argumentSeparator="^"/></a>
					<span class="remove">					
