    ...
    <div class="headerContent">
        <ul class="nav">
            ...
            <sec:authorize ifAnyGranted="ROLE_CUSTOMERGROUP">
                <li class="logged_in"><ycommerce:testId code="header_LoggedUser"><spring:theme code="header.welcome"
                                                                                               arguments="${user.firstName},${user.lastName}"
                                                                                               htmlEscape="true"/></ycommerce:testId></li>
            </sec:authorize>
            <sec:authorize ifNotGranted="ROLE_CUSTOMERGROUP">
                <li><ycommerce:testId code="header_Login_link"><a href="<c:url value="/login"/>"
                                                                  title="<spring:theme code="header.link.login"/>"><spring:theme
                        code="header.link.login"/></a></ycommerce:testId></li>
            </sec:authorize>
            <sec:authorize ifAnyGranted="ROLE_CUSTOMERGROUP">
                <li><ycommerce:testId code="header_myAccount"><a href="<c:url value="/my-account"/>"
                                                                 title="<spring:theme code="header.link.account"/>"><spring:theme
                        code="header.link.account"/></a></ycommerce:testId></li>
            </sec:authorize>
            <cms:slot var="link" contentSlot="${slots.HeaderLinks}">
                <li><cms:component component="${link}"/></li>
            </cms:slot>
            <li>| &nbsp; <a href="<c:url value="/store-finder"/>"
                            title="<spring:theme code="general.find.a.store" />"><spring:theme
                    code="general.find.a.store"/></a></li>
            <sec:authorize ifAnyGranted="ROLE_CUSTOMERGROUP">
                <li><ycommerce:testId code="header_signOut"><a href="<c:url value='/logout'/>"><spring:theme
                        code="header.link.logout"/></a></ycommerce:testId></li>
            </sec:authorize>
        </ul>
