        ...
        <ul id="carousel_alternate" class="jcarousel-skin jcarousel-vertical alt">
            <c:forEach items="${galleryImages}" var="container" varStatus="varStatus">
                <li>
					<span class="thumb">
						<a href="#">
                            <img src="${container.thumbnail.url}" data-primaryimagesrc="${container.product.url}"
    ...
    <div class="prod_image_main" id="primary_image">
        ...
        <ycommerce:testId code="productDetails_zoomImage_button">
            <a class="modal" id="zoomLink" href="${productZoomImagesUrl}" target="_blank">
                <span class="details"
                      style="display: block; position: absolute; bottom: 40px; right: 10px; width: 18px; height: 18px;"
                      title="<spring:theme code="general.zoom"/>"></span>
            </a>
        </ycommerce:testId>
    </div>
</div>
        ...
        <div class="prod_review">
            ...
            <p class="prod_review-new">
                <a href="#" id="write_review_action_main"
                   title="<spring:theme code="review.write.title" />"><spring:theme code="review.write.title"
                                                                                    arguments="${product.name}"/></a>
            </p>
        </div>
                                    ...
                                    <c:if test="${not empty variantSizes}">
                                        ...
                                        <c:forEach items="${variantSizes}" var="variantSize">
                                            ...
                                            <option value="${variantOptionUrl}" ${(variantSize.url eq product.url) ? 'selected="selected"' : ''}>
                                                    ...
                                                    priceData="${variantSize.priceData}"/>&nbsp;&nbsp;${stockLevel}
                                            </option>
                                        </c:forEach>
                                    </c:if>
                                </select>
