...
<div class="item_container_holder">
    ...
    <div class="item_container">
        ...
        <div class="cart_id"><spring:theme code="basket.page.cartId"/>&nbsp;<span
                class="cart-id-nr">${cartData.code}</span>
            <c:forEach var="invalidMap" items="${cartData.firstIncompleteBundleComponentsMap}">
                ...
                <div class="small_popup_container">
                    ...
                    <div class="small_popup_content">
                        <h3><spring:theme code="basket.page.message.edit0"
                                          arguments="${invalidMap.value.name},${invalidMap.key}"/></h3>

                        <div class="clear"></div>
                        <p><spring:theme code="basket.page.message.info"/></p>

                        <div class="invalid-bundle-wizard">
                            <a href="<c:url value="/bundle/edit-component/${invalidMap.key}/component/${invalidMap.value.id}"/>"
                               class="invalid-bundle-wizard-editable"><spring:theme code="basket.page.message.return"
                                                                                    arguments="${invalidMap.value.name}"/></a>
                        </div>
                        <div class="clear"></div>
                    </div>
                </div>
                        ...
                        <c:when test="${not empty entry.rootBundleTemplate.name}">
								    <span class="cart-bundle-name">
								    	...
								    	<c:if test="${displayPackageNo == true}">
                                            <span class="cart-bundle-number">(<spring:theme
                                                    code="basket.page.bundleNo"/> ${entry.bundleNo})</span>
                                        </c:if>
								    </span>
                            <c:url var="bundleDeleteAction" value="/cart/delete"/>
                            <form:form id="deleteBundleForm${uniqueBundleId}" action="${bundleDeleteAction}"
                                       method="post" commandName="deleteBundleForm${uniqueBundleId}">
            ...
            <tr class="cart-bundle-content">
                <td class="cart-bundle-package">
                    ...
                    <c:if test="${entry.editable}">
                        <c:if test="${entry.valid}">
                            <a href="<c:url value="/bundle/edit-component/${entry.bundleNo}/component/${entry.component.id}"/>"
                               class="cart-bundle-editable"><spring:theme code="basket.page.edit" text="Edit"/></a>
                        </c:if>
                    </c:if>
                ...
                <td class="cart-bundle-product">
                    ...
                    <ycommerce:testId code="cart_product_name">
                        ...
                        <div class="cart-productentitlements">
                    ...
                    <c:if test="${not entry.valid}">
                        <div class="invalid-cartitem">
                            <spring:theme code="basket.page.bundle.message.missing.option"
                                          text="Sorry, there seems to be a missing option."/>
                            <c:forEach var="invalidMap" items="#">
                                <c:if test="${entry.editable}">
                                    <a href="<c:url value="/bundle/edit-component/${entry.bundleNo}/component/${entry.component.id}"/>"
                                       class="cart-bundle-editable invalid-cartitem-editable"><spring:theme
                                            code="basket.page.message.addComponent" text="Please add {0}"
                                            arguments="${entry.component.name}"/></a>
                                </c:if>
                            </c:forEach>
                        </div>
                    </c:if>
                ...
                <td class="cart-bundle-quantity">
                    ...
                    <c:if test="${entry.product !=null }">
                        <form:form id="updateCartForm${uniqueFormId}" action="${cartUpdateFormAction}" method="post"
                                   commandName="updateQuantityForm${uniqueFormId}">
                            ...
                            <c:if test="${entry.updateable and !isSubscriptionProduct}">
                                <ycommerce:testId code="cart_product_updateQuantity">
                                    <a href="javascript:submitUpdate('${uniqueFormId}');"><spring:theme
                            ...
                            <ycommerce:testId code="cart_product_quantity">
                                <c:choose>
                                    ...
                                    <c:otherwise>
                                        ...
                                        <input type="text" name="quantity" class="qty" size="1"
                ...
                <c:choose>
                    <c:when test="${not empty entry.product}">
                        <c:forEach items="${entry.orderEntryPrices}"
                                   var="orderEntryPrice"> <!-- please ensure that these TDs get rendered always to avoid a uneven number of TDs per Row -->
                            <td class="cart-bundle-itemPrice">
                                <c:choose>
                                    <c:when test="${not empty orderEntryPrice.totalPrice}">
                                        <c:if test="${(orderEntryPrice.basePrice.value - orderEntryPrice.totalPrice.value) > 0}">
                                            <del><format:price priceData="${orderEntryPrice.basePrice}"
                    ...
                    <c:if test="${entry.removeable}">
                        <ycommerce:testId code="cart_product_removeProduct">
                            <a href="javascript:submitRemove('${uniqueFormId}');">
                                <spring:theme code="text.iconCartRemove" var="iconCartRemove"/>
                                <theme:image code="img.iconCartRemove" alt="${iconCartRemove}"
                                             title="${iconCartRemove}"/>
                            </a>
                        </ycommerce:testId>
                    </c:if>
                </td>
