...
<script id="paymentSummaryTemplate" type="text/x-jquery-tmpl">
				...
				<div class="left">
					<ul>
						{{if paymentInfo}}
							...
							<li><spring:theme code="checkout.summary.paymentMethod.paymentDetails.expires"
                                              arguments="{{= paymentInfo.expiryMonth}},{{= paymentInfo.expiryYear}}"/></li>
						{{else}}
					...
					{{if paymentInfo}}
						<form>
							<dl>
								<dt><label for="SecurityCode"><spring:theme
        code="checkout.summary.paymentMethod.securityCode"/>*</label></dt>
								<dd>
    ...
    <a href="#" class="edit_complete change_payment_method_button" payment_id="{{= paymentInfo.id}}" ><spring:theme
        code="checkout.summary.paymentMethod.editPaymentMethod"/></a>
</ycommerce:testId>
		{{else}}
			<ycommerce:testId code="checkout_changePayment_element">
    <button class="form change_payment_method_button" payment_id=""><spring:theme
        code="checkout.summary.paymentMethod.editPaymentMethod"/></button>
</ycommerce:testId>
		{{/if}}
	</div>
		...
		{{each savedCards}}
			<tr>
				...
				<ul>
					...
					<li><spring:theme code="checkout.summary.paymentMethod.paymentDetails.expires"
                                      arguments="{{= $value.expiryMonth}},{{= $value.expiryYear}}"/></li>
				</ul>
				</td>
				<td headers="header2">
					<ul>
				...
				<td headers="header3">
					<button class="form use_this_saved_card_button" payment_id='{{= $value.id}}'><spring:theme
        code="checkout.summary.paymentMethod.savedCards.UseThisSavedCard"/></button>
				</td>
			</tr>
		{{/each}}
	{{/if}}
        ...
        <c:choose>
            <c:when test="${paymentInfoOk}">
                <a href="#" class="edit_complete change_payment_method_button"
                   payment_id="${not empty paymentInfo ? paymentInfo.id : ''}"><spring:theme
                        code="checkout.summary.paymentMethod.editPaymentMethod"/></a>
            </c:when>
            <c:otherwise>
