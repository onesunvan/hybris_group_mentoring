        ...
        <ul class="table_action_col_item">
            <li>
                <c:choose>
                    ...
                    <c:otherwise>
                        <a id="deliveryAddress_select_link_${count}"
                           href="${selectDeliveryAddressUrl}/?selectedAddressCode=${deliveryAddress.id}"><spring:theme
                                code="checkout.multi.deliveryAddress.select" text="Select"/></a>
                    </c:otherwise>
                </c:choose>
            </li>
            <li>
                <a id="deliveryAddress_edit_link_${count}"
                   href="${editDeliveryAddressUrl}/?editAddressCode=${deliveryAddress.id}"><spring:theme
                        code="checkout.multi.deliveryAddress.edit"/></a>
            </li>
        </ul>
    </td>
    <td headers="header2">
