...
<%@ attribute name="paymentMethod" required="true"
        ...
        <ul class="table_action_col_item">
            <li>
                <c:choose>
                    ...
                    <c:otherwise>
                        <a id="paymentMethod_select_link_${count}"
                           href="${selectPaymentMethodUrl}/?selectedPaymentMethodId=${paymentMethod.id}"><spring:theme
                                code="checkout.multi.paymentMethod.savedCards.select"/></a>
                    </c:otherwise>
                </c:choose>
            </li>
        </ul>
    ...
    <td headers="header2">
        <ul>
            ...
            <li><spring:theme code="checkout.multi.paymentMethod.paymentDetails.expires"
                              arguments="${fn:escapeXml(paymentMethod.expiryMonth)},${fn:escapeXml(paymentMethod.expiryYear)}"/></li>
        </ul>
    </td>
    <td headers="header3">
        <ul>
            <c:if test="${billingAddressOk}">
                <li><c:if
                        test="${not empty paymentInfo.billingAddress.title}">${fn:escapeXml(paymentMethod.billingAddress.title)}&nbsp;</c:if>${fn:escapeXml(paymentMethod.billingAddress.firstName)}
                    &nbsp;${fn:escapeXml(paymentMethod.billingAddress.lastName)}</li>
