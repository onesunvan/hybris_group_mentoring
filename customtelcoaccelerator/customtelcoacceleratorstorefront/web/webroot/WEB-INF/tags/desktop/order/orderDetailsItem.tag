...
<div class="item_container_holder">
    ...
    <div class="item_container">
        ...
        <c:if test="${not empty order.status}">
            <p><spring:theme code="text.account.order.orderStatus" text="The order is {0}"
                             arguments="${order.statusDisplay}"/></p>
        </c:if>

        <table id="your_cart" class="your_cart-CartItems confirmation-CartItems">
            <thead>
            <tr>
                <th id="header1"><span class="hidden"><spring:theme code="basket.page.product" text="Product"/></span>
                </th>
                <th id="header2"><span class="hidden"><spring:theme code="basket.page.productdetails"
                                                                    text="Product Details"/></span></th>
                    ...
                    <c:choose>
                        <c:when test="${not empty entry.product}">
                            <c:forEach items="${entry.orderEntryPrices}" var="orderEntryPrice"
                                       varStatus="rowCounter"> <!-- please ensure that these TDs get rendered always to avoid a uneven number of TDs per Row -->
                                <td headers="header${3 + rowCounter.count}" class="cart-bundle-itemPrice">
                                    <c:choose>
