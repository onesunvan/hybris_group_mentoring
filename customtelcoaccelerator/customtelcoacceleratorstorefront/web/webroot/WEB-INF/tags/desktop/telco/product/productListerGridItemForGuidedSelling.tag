        ...
        <c:if test="${product.disabled}">
			<span class="product-disabled-message">
				<spring:theme code="product.list.cannotPurchaseReason"
                              text="Product cannot be purchased because of: {0}" arguments="${product.disabledMessage}"
                              argumentSeparator="!!!!"/>
			</span>
        </c:if>
        ...
        <div class="grid-product-price">
            <ycommerce:testId code="product_productPrice">
                <c:if test="${product.thisBundleProductPrice.value ne null and product.thisBundleProductPrice.value lt product.price.value}">
                    ...
                    <p class="price"><format:price priceData="${product.thisBundleProductPrice}"
                                                   displayFreeForZero="true"/></p>
                </c:if>
            ...
            <form:form id="bundleAddToCartForm${product.code}" action="${addToCartBundle}" method="post">
                <div id="${product.code}" class="select-device">
                    <button type="submit" class="positive small delayed-button"
                            title="<spring:theme code="guidedselling.select.device.select.button" />">
