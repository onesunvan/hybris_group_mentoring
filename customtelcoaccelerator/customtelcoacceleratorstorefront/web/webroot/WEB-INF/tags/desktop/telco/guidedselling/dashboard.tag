        ...
        <div class="dashboard-item <c:if test="${dashboardBox.active}">dashboard-item-active</c:if>	">
            ...
            <c:if test="${!dashboardBox.active and not empty dashboardBox.mainComponent and isBundleValid}">
                <div class="dashboard-edit">
                    <a href="<c:url value="/bundle/edit-component/${dashboard.bundleNo}/component/${dashboardBox.mainComponent.id}"/>"><spring:theme
                            code="guidedselling.dashboard.button.edit" text="Edit"/></a>
                </div>
            </c:if>
            ...
            <div class="dashboard-item-row-container">
                <c:forEach items="${dashboardBox.dashboardBoxEntries}" var="dashboardBoxEntry"
                           varStatus="dashboardBoxEntryCounter">
                    <div class="dashboard-item-row">
                        <c:if test="${not empty ycommerce:productImage(dashboardBoxEntry.orderEntry.product, 'thumbnail')}">
                            <div class="dashboard-item-thumb">
                                <product:productPrimaryImage product="${dashboardBoxEntry.orderEntry.product}"
                                                             format="thumbnail"/>
                            </div>
                        </c:if>
                        ...
                        <div class="dashboard-item-price">
                            ...
                            <c:choose>
                                <c:when test="${not empty dashboardBoxEntry.orderEntry.product.subscriptionTerm}">
                                    <c:forEach items="${dashboardBoxEntry.orderEntry.orderEntryPrices}"
                                               var="oderEntryPrice">
                                        <c:if test="${oderEntryPrice.defaultPrice}">
                                            <c:if test="${(oderEntryPrice.basePrice.value - oderEntryPrice.totalPrice.value) > 0}">
                                                <del>
                                                    <format:price priceData="${oderEntryPrice.basePrice}"
                                                                  displayFreeForZero="false"/>
                                                </del>
                                            </c:if>
                                            <format:price priceData="${oderEntryPrice.totalPrice}"
                                ...
                                <c:otherwise>
                                    <c:if test="${(dashboardBoxEntry.orderEntry.basePrice.value - dashboardBoxEntry.orderEntry.totalPrice.value) > 0}">
                                        <del>
                                            <format:price priceData="${dashboardBoxEntry.orderEntry.basePrice}"
                                                          displayFreeForZero="false"/>
                                        </del>
                                    </c:if>
                                    <format:price priceData="${dashboardBoxEntry.orderEntry.totalPrice}"
                                                  displayFreeForZero="true"/>
									<span class="dashboard-item-frequency">
