...
<div class="prod_add_to_cart">
    ...
    <div class="tabs">
        ...
        <c:forEach items="${bundleTabs}" var="bundleTab" varStatus="bundleTabCounter">
            ...
            <h2 <c:if
                    test="${bundleTab.preselected}"> id="preselected"</c:if>>${bundleTab.parentBundleTemplate.name}</h2> <!-- TODO please preselect -->
            <div class="tabbody">
                ...
                <div class="subTabs">
                    ...
                    <c:forEach items="${bundleTab.frequencyTabs}" var="frequencyTab" varStatus="frequencyTabCounter">
                        <h3 <c:if test="${frequencyTab.preselected}"> id="preselected_sub"</c:if>><c:if
                                test="${frequencyTab.termOfServiceNumber gt 0}">${frequencyTab.termOfServiceNumber} &nbsp;</c:if> ${frequencyTab.termOfServiceFrequency.name}</h3> <!-- TODO please preselect -->
                                ...
                                <table class="sub-tabbody-table">
                                    <tr>
                                        ...
                                        <c:forEach items="${frequencyTab.products}" var="plan" varStatus="plansCounter">
                                            <td
                                                    <c:if test="${selectProduct and plan.preselected}">class="plan-selected"</c:if>>${plan.name}</td>
                                        </c:forEach>
                                    </tr>
                                    <tr>
                                        <td><spring:theme code="product.list.viewplans.billingFrequency"
                                                          text="Billing Frequency"/></td>
                                        <c:forEach items="${frequencyTab.products}" var="plan" varStatus="plansCounter">
                                            <td
                                                    <c:if test="${selectProduct and plan.preselected}">class="plan-selected"</c:if>>${plan.subscriptionTerm.billingPlan.billingTime.name}</td>
                                        </c:forEach>
                                    </tr>
                                    <tr>
                                        ...
                                        <c:forEach items="${frequencyTab.products}" var="plan" varStatus="plansCounter">
                                            <td
                                                    <c:if test="${selectProduct and plan.preselected}">class="plan-selected"</c:if>>
                                                <c:if test="${not empty plan.price.recurringChargeEntries}">
                                                    ...
                                                    <c:forEach items="${plan.price.recurringChargeEntries}"
                                                               var="recurringPrice" varStatus="recurringPricesCounter">
                                                        <c:choose>
                                                ...
                                                <c:if test="${not empty plan.price.oneTimeChargeEntries}">
                                                    ...
                                                    <c:forEach items="${plan.price.oneTimeChargeEntries}"
                                                               var="oneTimePrice" varStatus="oneTimePricesCounter">
                                                        <c:if test="${not oneTimePricesCounter.first}">
                                                            <br/>
                                                        </c:if>
                                                        <spring:theme code="product.list.viewplans.price.onetime"
                                    ...
                                    <tr>
                                        <td><spring:theme code="product.list.viewplans.termOfServiceFrequency"
                                                          text="Term of service frequency"/></td>
                                        <c:forEach items="${frequencyTab.products}" var="plan" varStatus="plansCounter">
                                            <td
                                                    <c:if test="${selectProduct and plan.preselected}">class="plan-selected"</c:if>>
                                                <c:if test="${plan.subscriptionTerm.termOfServiceNumber gt 0}">${plan.subscriptionTerm.termOfServiceNumber} &nbsp;</c:if>
                                                ${plan.subscriptionTerm.termOfServiceFrequency.name}</td>
                                        </c:forEach>
                                    </tr>
                                    <tr>
                                        <td><spring:theme code="product.list.viewplans.termOfServiceRenewal"
                                                          text="Term of service renewal"/></td>
                                        <c:forEach items="${frequencyTab.products}" var="plan" varStatus="plansCounter">
                                            <td
                                                    <c:if test="${selectProduct and plan.preselected}">class="plan-selected"</c:if>>${plan.subscriptionTerm.termOfServiceRenewal.name}</td>
                                        </c:forEach>
                                    </tr>
                                    <tr>
                                        <td>
                                            <spring:theme code="product.list.viewplans.entitlements"
                                                          text="Entitlements"/>
                                        </td>
                                        <c:forEach items="${frequencyTab.products}" var="plan" varStatus="plansCounter">
                                            <td
                                    ...
                                    <tr>
                                        <td>
                                            <spring:theme code="product.list.viewplans.usage.charges"
                                                          text="Usage Charges"/>
                                        </td>
                                        <c:forEach items="${frequencyTab.products}" var="plan" varStatus="plansCounter">
                                            <td
                                                    ...
                                                    <c:forEach items="${plan.price.usageCharges}" var="usageCharge">
                                                        <div class="featureClass">
                                                            ...
                                                            <c:if test="${not empty usageCharge.usageChargeEntries}">
                                                                <c:forEach items="${usageCharge.usageChargeEntries}"
                                    ...
                                    <c:if test="${showButtons}">
                                        <tr>
                                            <td>&nbsp;</td>
                                            <c:forEach items="${frequencyTab.products}" var="plan"
                                                       varStatus="plansCounter">
                                                    ...
                                                    <form:form action="${addToCartBundle}" method="post">
                                                        ...
                                                        <div id="${bundleTab.parentBundleTemplate.id}_${plan.code}">
                                                            <c:if test="${plan.stock.stockLevelStatus == 'inStock'}">
                                                                <button type="submit"
                                                                        class="positive small delayed-button"
                                                        ...
                                                        <input type="hidden" name="bundleTemplateId"
                                    ...
                                    <c:forEach items="${frequencyTab.products}" var="plan" varStatus="plansCounter">
                                        <tr>
                                            ...
                                            <td
                                                ...
                                                <c:forEach items="${plan.price.recurringChargeEntries}"
                                                           var="curRecurringCharge"
                                                           varStatus="rowCounterRecurringCharge">
                                                    <c:set var="lastRecurringCharge" value="${curRecurringCharge}"/>
                                                </c:forEach>
                                                <format:price priceData="${lastRecurringCharge.price}"/>
                                                <div class="pay">${plan.subscriptionTerm.billingPlan.billingTime.name}</div>
                                            </td>
                                            <td
                                                    <c:if test="${selectProduct and plan.preselected}">class="plan-selected"</c:if>>
                                            ...
                                            <c:if test="${showButtons}">
                                                <form:form
                                                        id="bundleAddToCartForm${bundleTabCounter.count}_${plansCounter.count}"
                                                        action="${addToCartBundle}" method="post">
                                                    <td class="sub-tabbody-table-td4 <c:if test="${selectProduct and plan.preselected}">plan-selected</c:if>">
                                                        <div id="${bundleTab.parentBundleTemplate.id}_${plan.code}">
                                                            <button type="submit" class="positive small delayed-button"
                                                                    title="<spring:theme code="basket.add.to.basket" />">
                                                                <spring:theme code="basket.add.to.basket"/>
                                                            </button>
                                                        </div>
