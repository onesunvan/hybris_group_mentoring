...
<div class="prod_add_to_cart">
    ...
    <c:forEach items="${bundleTemplateData.bundleBoxes}" var="bundleBox" varStatus="bundleBoxCounter">
        <div class="prod_add_to_cart_box" <c:if test="${bundleBox.expanded}">id="box_expanded" tabindex="0"</c:if>>
            <h2>${bundleBox.component.name}
                <c:if test="${bundleBox.reviewButton}">
                    <a class="review_area"
                       href="<c:url value="/bundle/edit-component/${bundleNo}/component/${bundleBox.component.id}"/>"><spring:theme
                            code="extras.review.button"/></a>
                </c:if>
            </h2>
            ...
            <table class="prod_add_bundle-table">
                ...
                <c:forEach items="${bundleBox.bundleBoxEntries}" var="bundleBoxEntry" varStatus="bundleBoxEntryCounter">
                    <tr class="add_on <c:if test='${not bundleBoxEntry.selected and not bundleBoxEntry.disabled}'>remove_selected</c:if> <c:if test='${bundleBoxEntry.selected}'>selected</c:if>">
                        <form:form id="bundleAddToCartForm${bundleBoxEntryCounter.count}" action="${addToCart}"
                                   method="post">
                        ...
                        <td class="prod_add_to_cart-table-td2">
                            <c:if test="${not bundleBoxEntry.selected}">
                                <p class="expand_price"><format:price
                                        priceData="${bundleBoxEntry.product.thisBundleProductPrice}"/></p>
                                ...
                                <c:if test="${bundleBoxEntry.product.price['class'].simpleName eq 'SubscriptionPricePlanData' and not empty bundleBoxEntry.product.price.recurringChargeEntries}">
                                    <c:forEach items="${bundleBoxEntry.product.price.recurringChargeEntries}"
                                               var="curRecurringCharge" varStatus="rowCounterRecurringCharge">
                                ...
                                <p>
                                    <c:if test="${(lastRecurringCharge.price.value - bundleBoxEntry.product.thisBundleProductPrice.value) > 0}">
                                        <del><format:price priceData="${lastRecurringCharge.price}"
                            ...
                            <c:if test="${bundleBoxEntry.selected}">
                                <c:forEach items="${bundleBoxEntry.orderEntry.orderEntryPrices}" var="orderEntryPrice">
                                    <c:if test="${orderEntryPrice.defaultPrice}">
                                        <p class="expand_price"><format:price
                                                priceData="${orderEntryPrice.totalPrice}"/></p>
                            ...
                            <div class="frequency">
                                / ${bundleBoxEntry.product.subscriptionTerm.billingPlan.billingTime.name}</div>
                        ...
                        <td class="prod_add_to_cart-table-td3">
                            <c:if test="${not bundleBoxEntry.selected and not bundleBoxEntry.disabled}">
                                ...
                                <button type="${buttonType}" class="positive small"
                            ...
                            <c:if test="${bundleBoxEntry.selected and bundleBoxEntry.removable}">
                                <form:form id="updateCartForm${uniqueFormId}" action="${cartUpdateFormAction}"
                                           method="post">
                                    <input type="hidden" name="entryNumber"
                                           value="${bundleBoxEntry.orderEntry.entryNumber}"/>
                                    <input type="hidden" name="productCode"
                                           value="${bundleBoxEntry.orderEntry.product.code}"/>
                                    <input type="hidden" name="initialQuantity"
                                           value="${bundleBoxEntry.orderEntry.quantity}"/>
                                    <input type="hidden" name="bundleNo" value="${bundleNo}"/>
                                    ...
                                    -->
                                    <button type="${buttonType}" class="negative small"
                                            id="REMOVE_${bundleBoxEntry.orderEntry.product.code}"
                                            title="<spring:theme code="basket.icon.cart.remove" />">
        ...
        <c:if test="${bundleBox.expanded}">
            <div class="prod_add_to_cart_box_buttons">
                <p><a class="end_step"
                      href="<c:url value="/bundle/edit-component/${bundleNo}/nextcomponent/${bundleBox.component.id}"/>"><spring:theme
                        code="extras.continue.button"/></a></p>
    ...
    <p class="imdone_step-container"><a class="imdone_step" href="<c:url value="/cart"/>"><spring:theme
            code="extras.done.button"/></a></p>
</div>
