        ...
        <div class="item_container_holder subscriptions">
            ...
            <div class="item_container">
                <c:if test="${not empty subscriptions}">
                    <p><spring:theme code="text.account.subscriptions.manageSubscriptions"
                                     text="Manage your subscriptions"/></p>

                    <table id="subscriptions">
                        <thead>
                        <tr>
                            <th id="header1"><spring:theme code="text.account.subscriptions.productName"
                                                           text="Product Name"/></th>
                            <th id="header2"><spring:theme code="text.account.subscriptions.startDate"
                                                           text="Start Date"/></th>
                            <th id="header3"><spring:theme code="text.account.subscriptions.endDate"
                                                           text="End Date"/></th>
                            <th id="header4"><spring:theme code="text.account.subscriptions.status" text="Status"/></th>
                            <th id="header5"><spring:theme code="text.account.subscriptions.paymentDetails"
                                                           text="Payment Details"/></th>
                            <th id="header6"><spring:theme code="text.account.orderHistory.actions"
                                                           text="Actions"/></th>
                        </tr>
                        </thead>
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td headers="header2" class="background">
                                    <ycommerce:testId code="subscriptions_startDate_label">
                                        <p><fmt:formatDate value="${subscription.startDate}" dateStyle="long"
                                                           timeStyle="short" type="date"/></p>
                                    </ycommerce:testId>
                                </td>
                                <td headers="header3">
                                    <ycommerce:testId code="subscriptions_endDate_label">
                                        <p><fmt:formatDate value="${subscription.endDate}" dateStyle="long"
                                                           timeStyle="short" type="date"/></p>
                                    </ycommerce:testId>
                                </td>
                                ...
                                <td headers="header5">
                                    <ycommerce:testId code="subscriptions_status_label">
                                        <c:if test="${not empty paymentInfoMap[subscription.paymentMethodId]}">
                                            <c:set value="${paymentInfoMap[subscription.paymentMethodId]}"
                                                   var="paymentMethod"></c:set>
                                            <p title="<spring:theme code="text.account.subscriptions.payment.details.title" arguments="${paymentMethod.cardTypeData.name},${fn:replace(paymentMethod.cardNumber,'****','**** ')},${paymentMethod.expiryMonth},${paymentMethod.expiryYear}" text=""/>">
                                                <spring:theme code="text.account.subscriptions.payment.details"
                                                              arguments="${fn:substring(paymentMethod.cardTypeData.name,0,4)},${fn:replace(paymentMethod.cardNumber,'*','')},${paymentMethod.expiryMonth},${paymentMethod.expiryYear}"
                                                              text=""/></p>
                                        </c:if>
                                    </ycommerce:testId>
                                </td>
                                <td headers="header6" class="background">
                                    <button type="button" onclick="window.location='${myAccountSubscriptionDetailsUrl}'"
                                            class="function_btn">
                                    ...
                                    <c:if test="${fn:toUpperCase(subscription.subscriptionStatus) ne 'PAUSED' and fn:toUpperCase(subscription.subscriptionStatus) ne 'CANCELLED'
                                        ...
                                        <c:url value="/my-account/upgradeSubscriptionComparison?subscriptionId=${subscription.id}"
                                               var="upgradeSubscriptionComparisionUrl"/>
                                        <button type="button"
                                                onclick="window.location='${upgradeSubscriptionComparisionUrl}'"
                                                class="positive">
                                            <spring:theme code="text.account.subscription.upgradeSubscription"
                ...
                <c:if test="${empty subscriptions}">
                    <p><spring:theme code="text.account.subscriptions.noSubscriptions"
                                     text="You have no subscriptions"/></p>
                </c:if>
            </div>
        </div>
    </div>
