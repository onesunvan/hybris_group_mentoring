            ...
            <div class="item_container">
                <c:if test="${not empty grants}">
                    ...
                    <table id="entitlements">
                        <thead>
                        <tr>
                            <th id="header1"><spring:theme code="text.account.entitlements.entitlementName"
                                                           text="Entitlement Name"/></th>
                            <th id="start"><spring:theme code="text.account.entitlements.date.start"
                                                         text="start date"/></th>
                            ...
                            <tr>
                                ...
                                <td headers="start">
                                    <ycommerce:testId code="redundant">
                                        <p><fmt:formatDate value="${grant.startTime}" dateStyle="long" timeStyle="short"
                                                           type="date"/></p>
                                    </ycommerce:testId>
                                </td>
                                <td headers="end">
                                    <ycommerce:testId code="redundant">
                                        <c:choose>
                                            <c:when test="${null == grant.endTime}">
                                                <p><spring:theme code="text.account.entitlements.date.end.unlimited"
                                                                 text="Unlimited"/></p>
                                            </c:when>
                                            <c:otherwise>
                                                <p><fmt:formatDate value="${grant.endTime}" dateStyle="long"
                ...
                <c:if test="${empty grants}">
                    <p><spring:theme code="text.account.entitlements.noEntitlements"
                                     text="You have no entitlements"/></p>
                </c:if>
            </div>
        </div>
    </div>
